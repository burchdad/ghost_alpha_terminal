...
export default function DecisionAuditPanel({ entries }: { entries: AuditSummary[] | null }) {
  if (!entries) {
  ...
  return (
    <div className="panel animate-riseIn rounded-xl p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-300">Immutable decision lineage</span>
      </div>
        ...
        <table className="min-w-full text-left text-xs">
          ...
          <tbody>
            ...
            {entries.slice(0, 8).map((item) => (
              <tr key={item.audit_id} className="border-t border-terminal-line">
