...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    const next = new URL("/brokerages", url.origin);
    next.searchParams.set("schwab_oauth", "error");
  ...
  try {
    const exchangeRes = await fetch(
      `${backendUrl}/auth/schwab/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`,
      {
        method: "GET",
        cache: "no-store",
        redirect: "manual",
    ...
    const nextPath = payload?.next && payload.next.startsWith("/") ? payload.next : "/brokerages";
    const next = new URL(nextPath, url.origin);
    next.searchParams.set("schwab_oauth", payload?.connected ? "connected" : "error");
    return NextResponse.redirect(next);
  } catch (err) {
    const next = new URL("/brokerages", url.origin);
    next.searchParams.set("schwab_oauth", "error");
